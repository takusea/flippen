...
impl Tool for FillTool {
    fn apply(&mut self, frame: &mut Image, x: u32, y: u32, color: Color, _pressure: Option<f32>) {
        let idx = ((y as usize) * (frame.width as usize) + (x as usize)) * 4;
        if idx + 3 >= frame.data.len() {
            return;
        }
        let target_color = {
            let slice = &frame.data[idx..idx + 4];
        ...
        let mut visited = vec![false; frame.width as usize * frame.height as usize];
        ...
        while let Some((cx, cy)) = stack.pop() {
            ...
            let i = ((cy as usize) * (frame.width as usize) + (cx as usize)) * 4;
            let vi = (cy as usize) * (frame.width as usize) + (cx as usize);
            ...
            if color_distance((&frame.data[i..i + 4]).try_into().unwrap(), &target_color)
                > self.tolerance
            {
                continue;
            }

            frame.data[i..i + 4].copy_from_slice(&color);
